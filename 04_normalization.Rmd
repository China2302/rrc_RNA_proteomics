...
samples_rna_prot<- sample_data_4M %>% inner_join(samples_prot,by="UniqueID")
...
### Eliminating samples collected in sample period 1
```{r}
sample_data_4M_nosp1<- sample_data_4M %>% filter(is.na(SP1))

list_nosp1<- sample_data_4M_nosp1$Sample_ID


count_data_noSP1<- as.data.frame(count_data_all)

count_data_noSP1<- count_data_noSP1[,(colnames(count_data_noSP1) %in% list_nosp1)]

count_data_noSP1<- count_data_noSP1[,(colnames(count_data_noSP1) %in% rownames(sample_data_4M_nosp1))]

all(rownames(sample_data_4M_nosp1) %in% colnames(count_data_noSP1))
all(rownames(sample_data_4M_nosp1) == colnames(count_data_noSP1))

```

### Eliminating samples from North ECA 
```{r}
# The North ECA location doesn't have any SCTLD unaffected colonies
sample_data_4M_noNorth<- sample_data_4M_nosp1 %>% filter(is.na(North))

list_noNorth<- sample_data_4M_noNorth$Sample_ID #118 samples


count_data_noNorth<- as.data.frame(count_data_noSP1)


count_data_noNorth<- count_data_noNorth[,(colnames(count_data_noNorth) %in% list_noNorth)]

count_data_noNorth<- count_data_noNorth[,(colnames(count_data_noNorth) %in% rownames(sample_data_4M_noNorth))]

all(rownames(sample_data_4M_noNorth) %in% colnames(count_data_noNorth))
all(rownames(sample_data_4M_noNorth) == colnames(count_data_noNorth))
```
...
ggplot(samples.to.be.excluded.reads.meta, aes(x = season, fill = Region)) +
...
colData.reads <- samples_rna_prot  %>% #column_to_rownames(var = 'Sample_ID') %>% 
  filter(!row.names(.) %in% samples.to.be.excluded.reads) #98
```
...
```{r}
pcaData_outliers = plotPCA(rlog_dds_dist, intgroup=c("colnames",'SCTLD_RNA',"sample_month","Location","time_point","Region","ID","Sample_ID","season","SP_region","SCTLD_RNA_Location_season","month_location"),
returnData=TRUE)
...
suppressWarnings({ 
  ggplot(pcaData_outliers, aes(PC1, PC2, colour = SP_region, shape = SCTLD_RNA)) +
...
```
```{r}
pcaData_outliers$group <- pcaData_outliers$season

ggplot(pcaData_outliers, aes(PC1, PC2, colour = SP_region, shape = SCTLD_RNA)) +
  geom_point(size = 2) +
  stat_ellipse(aes(group = SP_region), level = 0.95, type = "norm") +
  geom_text_repel(aes(label = Sample_ID), nudge_x = -1, nudge_y = 0.2, size = 4, max.overlaps = 100) +
  theme_bw() +
  ggtitle("Principal Component Analysis ~ season, dds_outliers", subtitle = "rlog") +
  xlab(paste0("PC1: ", percentVar_outliers[1], "% variance")) +
...
highlight_samples.den <- c("224_663","148_MC019","192_1280","212_N48","212_699","140_LC077","216_3446","184_1463","267_713","208_713","196_1452","287_693","257_741")
...
```{r}
samples.to.be.excluded.den <- c("224_663","148_MC019","192_1280","212_N48","212_699","140_LC077","216_3446","184_1463","267_713","208_713","196_1452","287_693","257_741")
...
# Assuming you have a vector of sample names corresponding to each row in pca_scores
sample_names <- rownames(pca_scores)  # or another vector of your sample names

# Plot the 3D PCA plot
plot3d(pca_scores[, 1], pca_scores[, 2], pca_scores[, 3], 
       col = "blue", size = 5, 
       xlab = "PC1", ylab = "PC2", zlab = "PC3", 
       main = "3D PCA Plot")

# Add text labels for all the samples
text3d(pca_scores[, 1], pca_scores[, 2], pca_scores[, 3], 
       texts = sample_names, 
       cex = 0.6, col = "black")  # Adjust cex to make text smaller if needed


```

### 3D PCA plot to visualized outliers
```{r}
pca_scores <- rpca1@scores

outliers_3dpca<- c("224_N53","317_LC085","327_MC002","247_1280","252_3342","322_LC016","282_689","232_N50","272_N58","232_N46","297_LC127","307_MC014","140_LC077","224_663","208_N52","208_713","192_1280","212_N48")

# Verify that pca_scores has rownames
if (is.null(rownames(pca_scores))) {
  stop("The 'pca_scores' object must have row names corresponding to sample names.")
}

# Check for valid outliers in pca_scores
valid_outliers <- outliers_3dpca[outliers_3dpca %in% rownames(pca_scores)]
...
if (length(valid_outliers) > 0) {
  # Extract coordinates for the valid outliers
  outlier_coords <- pca_scores[valid_outliers, , drop = FALSE]
  
  # Add red points for outliers
  points3d(outlier_coords[, 1], outlier_coords[, 2], outlier_coords[, 3],
           col = "red", size = 8)
  
  # Label outliers by their sample names
  text3d(outlier_coords[, 1], 
         outlier_coords[, 2], 
         outlier_coords[, 3], 
         texts = rownames(outlier_coords), 
         cex = 1, col = "black")
} else {
  cat("No matching outliers found in the row names of pca_scores.\n")
}
...
```{r}
samples.to.be.excluded.3dpca <- c("224_N53","317_LC085","327_MC002","247_1280","252_3342","322_LC016","282_689","232_N50","272_N58","232_N46","297_LC127","307_MC014","140_LC077","224_663","208_N52","208_713","192_1280","212_N48")
samples.to.be.excluded.3dpca.meta <- colData.reads  %>% #column_to_rownames(var = 'Sample_ID') %>% 
  filter(row.names(.) %in% samples.to.be.excluded.3dpca) 

ggplot(samples.to.be.excluded.3dpca.meta, aes(x = season, fill = Location)) +
  geom_bar(position = "dodge") +
  geom_text(stat = 'count', aes(label = ..count..),
            position = position_dodge(width = 0.9), vjust = -0.5, size = 3) +
  facet_wrap(~ SCTLD_RNA) +
...
outliers_log10<- c("208_N56","297_LC018","317_LC085","312_LC088","297_LC127","247_1280","237_1332","252_3342","232_N60","272_N58","277_679","282_689")
# Step 2: Create a color vector to highlight outliers in red, others in gray
sample_names <- colnames(count_data.reads)
highlight_colors <- ifelse(sample_names %in% outliers_log10, "red", "gray")


# Step 3: Create the boxplot for log-transformed counts
par(mgp = c(3, 0.5, 0))  # Adjust margin placement
boxplot(log10(counts(dds_dist, normalized = TRUE) + 1),
        main = "Log10 Counts per Sample",
        ylab = "Log10(Counts + 1)",
        las = 2,  # Rotate labels to vertical
        cex.axis = 0.8,  # Adjust font size
        col = highlight_colors)  # Highlight outlier samples in red
...
```{r}
samples.to.be.excluded.log10 <- c("208_N56","297_LC018","317_LC085","312_LC088","297_LC127","247_1280","237_1332","252_3342","232_N60","272_N58","277_679","282_689")
samples.to.be.excluded.log10.meta <- colData.reads  %>% #column_to_rownames(var = 'Sample_ID') %>% 
  filter(row.names(.) %in% samples.to.be.excluded.log10) 

ggplot(samples.to.be.excluded.log10.meta, aes(x = season, fill = Location)) +
  geom_bar(position = "dodge") +
  geom_text(stat = 'count', aes(label = ..count..),
            position = position_dodge(width = 0.9), vjust = -0.5, size = 3) +
  facet_wrap(~ SCTLD_RNA) +
...
sample_labels <- colData.reads$SCTLD_RNA_Region_season
...
high_distance_within_group <- subset(
  distance_df_full,
  Category == "Within Group" & Distance > 15000
)
...
cat("Number of within-group pairs with distance > 15000:", num_high_distance_pairs, "\n")
