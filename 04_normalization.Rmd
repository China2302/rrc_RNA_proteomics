...
samples_rna_prot<- samples_rna_prot %>% 
              ...
              mutate(count_postdiseased = Grouping == "post_sampling") %>% 
...
ggplot(samples_rna_prot, aes(x = season, fill = Grouping)) +
  ...
  theme_light() +
  labs(title = "Sample Distribution for Proteomic and RNA analysis",
       subtitle = "Across Seasons and SCTLD Grouping",
       x = "Season",
       y = "Number of Samples",
       fill = "Grouping") +
  theme(
...
## RESULT: Before outlier removal, all samples left are from Lower Keys. Groupings that can be analyse with RNA data will be "active_disease", "less_6months_post_sampling", "
```
### Removing genes with low reads 
```{r}
number_prediseased <- sum(samples_rna_prot$count_postdiseased == "TRUE") #26 samples
...
ggplot(df, aes(x = total_counts)) +
  ...
  geom_vline(xintercept = 2e6, color = "red", linetype = "dashed", size = 1) +
  annotate(
    "text",
    x = log10(2e6),
...
ggplot(df, aes(x = log10(total_counts))) +
  geom_density(fill = "lightblue") +
  geom_vline(xintercept = log10(2e6), color = "red", linetype = "dashed", size = 1) +
  annotate(
    "text",
    x = log10(2e6),
...
samples.to.be.excluded.reads <- names(total_counts_per_sample[total_counts_per_sample < 2e6])
...
```{r}
# Assuming 'total_counts_per_sample' contains the total counts for each sample
samples.to.be.excluded.reads_high <- names(total_counts_per_sample[total_counts_per_sample > 5e6])
...
```{r}
samples.to.be.excluded.reads_high.meta <- samples_rna_prot  %>% #column_to_rownames(var = 'Sample_ID') %>% 
...
ggplot(colData.reads, aes(x = Grouping, fill = Grouping)) +
  geom_bar(position = "dodge") +
  geom_text(stat = 'count', aes(label = ..count..),
            position = position_dodge(width = 0.9), vjust = -0.5, size = 3) +
  facet_wrap(~ Location) +
...
### Modelling data with DESEq2 with full model : ~ Grouping
```{r}
...
suppressWarnings({ 
  ggplot(pcaData_outliers, aes(PC1, PC2, colour = Grouping, shape = season)) +
...
```{r}
samples.to.be.excluded.rlog <- c("208_N52","267_713")
...
#                                  NA)
pcaData_idsamples$outlier <- ifelse(pcaData_idsamples$Sample_ID %in% highlight_samples, "outlier", "no-outlier")
...
# Create the PCA plot
ggplot(pcaData_idsamples, aes(PC1, PC2, colour = outlier, shape = season)) +
  geom_point(aes(color = outlier), size = 4) + 
  theme_bw() +
  # Show labels only for highlighted samples
  geom_text_repel(data = subset(pcaData_idsamples, outlier == "outlier"), 
                  aes(label = Sample_ID), 
  ...
  # scale_y_continuous(breaks = seq(-50, 30, by = 5), limits = c(-50, 30)) +
  scale_color_manual(values = c("outlier" = "red", "no-outlier" = "black"))  # Customize colors
...
```{r}
samples.to.be.excluded.rpca <- c("63_N56")
...
grp_names_dist <- dds_dist@colData$Grouping
...
rownames(sampleDistMatrix_dist) <- grp_names_dist
...
sample_labels <- colData.reads$Grouping_season
...
high_distance_within_group <- subset(
  distance_df_full,
  Category == "Within Group" & Distance > 25000
)
...
highlight_samples <- c("208_N56")  # Replace with the actual sample names
...
```{r}
samples.to.be.excluded.manh <- c("208_N56")
...
```{r}
samples.to.be.excluded.comb <- c("208_N56")
# "322_LC016","208_N52","224_663","212_699","267_713","287_693","208_N56","297_LC018"
...
ggplot(colData.comb, aes(x = Grouping, fill = Grouping)) +
  geom_bar(position = "dodge") +
  geom_text(stat = 'count', aes(label = ..count..),
            position = position_dodge(width = 0.9), vjust = -0.5, size = 3) +
  # facet_wrap(~ Grouping) +
  scale_fill_viridis_d(option = "plasma") +
  theme_light() +
  labs(title = "Sample Distribution of sample",
       subtitle = "Across Seasons, Regions, and SCTLD_RNA Status",
       x = "Season",
       y = "Number of Samples",
       fill = "Location") +
...
### PLOT: PCA DESeq2 plot with rlog transformation
```{r}
pcaData_idgenes = plotPCA(vst_dds_idgenes, intgroup=c("colnames",'SCTLD_RNA',"sample_month","Location","time_point","Region","ID","Sample_ID","season","SP_region","Grouping"),
returnData=TRUE)
...
# png("results/3_All_4M_Norm/PCA_dds_null.filtered2_rlog.png", width=8, height=6, units = "in", res = 300)
ggplot(pcaData_idgenes, aes(PC1, PC2, colour = Grouping, shape = season)) +
geom_point(size = 4) + theme_bw() +
geom_text_repel(aes(label = season), nudge_x = -1, nudge_y = 0.2, size = 4) +
ggtitle("Principal Component Analysis ~ Grouping, dds_idgenes", subtitle = "rlog") +
...
genes_of_interest_idgenes <- c("QW917_004045","QW917_008916","QW917_016428","QW917_017250","QW917_021160","QW917_036527","QW917_026837","QW917_028695","QW917_029215","QW917_030073","QW917_035307","QW917_033408") 
...
ggplot(plot_data_idgenes, aes(x = Grouping, y = Expression, fill = Grouping)) +
...
genes.to.be.excluded <- c("QW917_004045","QW917_016428","QW917_021160","QW917_036527","QW917_026837","QW917_029215","QW917_035307","QW917_033408")
...
```{r}
ggplot(colData_4M, aes(Grouping)) +
...
```{r}
for_plot<- group_by(colData_4M,y="Grouping")

ggplot(for_plot, aes(Grouping)) +
