...
```{r}
dds_grouping<- DESeqDataSetFromMatrix(countData = data.counts, 
...
#### Fitting models
```{r}
if (file.exists("cache/dds_grouping.rds")) {
  dds_grouping <- read_rds("cache/dds_grouping.rds")
} else {
 dds_grouping <- DESeq(dds_grouping, parallel = TRUE)
  write_rds(dds_grouping,"cache/dds_grouping.rds")
}
...
```{r}
vst_dds_grouping = vst(dds_grouping, blind=FALSE)
...
if (file.exists("cache/rlog_dds_grouping.rds")) {
  rlog_dds_grouping <- read_rds("cache/rlog_dds_grouping.rds")
} else {
rlog_dds_grouping <- rlog(dds_grouping, blind=FALSE)
  write_rds(rlog_dds_grouping,"cache/rlog_dds_grouping.rds")
}
...
# png("results/5_All_4M_DEG/cookdistance_dds_grouping.png", width=30, height=8, units = "in", res = 300)
boxplot_dds_grouping<-boxplot(log10(assays(dds_grouping)[["cooks"]]+1), names = colData_4M$colnames,cex.axis = 0.7, las = 3,range=2,main="Cook's distance ~ SCTLD_RNA_Region_season")
# dev.off()

boxplot_dds_grouping<-boxplot(log10(assays(dds_grouping)[["cooks"]]), names = colData_4M$colnames,cex.axis = 0.7, las = 3,range=2,main="Cook's distance ~ SCTLD_RNA_Region_season")
beep()
```
...
# png("results/5_All_4M_DEG/Dispersionplot_dds_grouping.png", width=15, height=6, units = "in", res = 300)
plotDispEsts(dds_grouping, main="Dispersion plot dds_grouping ~ SCTLD_RNA_Region_season")
# dev.off()
...
#### PCA DESeq2 plot with vst transformation dds_grouping
```{r}
#PCA dds_grouping:~ SCTLD_RNA_Region_season

pcaData = plotPCA(vst_dds_grouping, intgroup=c("colnames",'SCTLD_RNA',"Region","sample_month","time_point","Location", "sample_day","percent_uniq", "season","Grouping"), 
returnData=TRUE)
percentVar = round(100 * attr(pcaData, "percentVar"))

# png("results/5_All_4M_DEG/PCA_dds_grouping_vst.png", width=10, height=8, units = "in", res = 300)
ggplot(pcaData, aes(PC1, PC2, colour = Grouping, shape =season )) + 
geom_point(size = 2) + theme_bw() + 
geom_text_repel(aes(label = Region), nudge_x = -1, nudge_y = 0.2, size = 3) +
ggtitle("Principal Component Analysis vst", subtitle = "~ Grouping") +
...
pcaData_rlog = plotPCA(rlog_dds_grouping, intgroup=c("colnames",'SCTLD_RNA',"Region","sample_month","time_point","Location","Grouping"), 
returnData=TRUE)
percentVar_rlog = round(100 * attr(pcaData_rlog, "percentVar"))

# png("results/5_All_4M_DEG/PCA_dds_grouping_rlog.png", width=10, height=8, units = "in", res = 300)
ggplot(pcaData_rlog, aes(PC1, PC2, colour = Location, shape = Grouping)) +
geom_point(size = 2) + theme_bw() +
geom_text_repel(aes(label = sample_month), nudge_x = -1, nudge_y = 0.2, size = 3) +
ggtitle("Principal Component Analysis rlog ", subtitle = "~ Grouping") +
...
#Results dds_grouping:~ SCTLD_RNA_Region_season
res_grouping<- results(dds_grouping)
...
- Factor names dds_grouping:~ SCTLD_RNA_Region_season
```{r}
resultsNames(dds_grouping)
```
...
```{r}
res_active_vs_never<- results(dds_grouping,contrast =
                                        
                              c("Grouping",'active_disease',"never_diseased"))

resOrdered_active_vs_never<- as.data.frame(
               
                                 res_active_vs_never[order(res_active_vs_never$padj),])
                
DEG_active_vs_never <-resOrdered_active_vs_never %>%  
  
                                mutate(ofav_gene = rownames(resOrdered_active_vs_never)) %>%
   
                                filter(padj<0.1)

DEG_active_vs_never_anno<- DEG_active_vs_never %>% left_join(anno, by="ofav_gene")
```

```{r}
write_csv(DEG_active_vs_never_anno, "data/DEG_active_vs_never.csv")
```

##### DEG btw SCTLD_affected vs SCTLD_ unaffected in winter --> winter
```{r}
res_lower_affected_winter_vs_unaffected_winter<- results(dds_grouping,contrast =
...
##### DEG btw SCTLD_affected vs SCTLD_ unaffected in fall --> fall
```{r}
res_ECA_affected_fall_vs_unaffected_fall<- results(dds_grouping,contrast =
...
##### DEG btw SCTLD_affected vs SCTLD_ unaffected in winter --> winter
```{r}
res_ECA_affected_winter_vs_unaffected_winter<- results(dds_grouping,contrast =
...
lower_fall_log.padj<- resOrdered_active_vs_never %>%
...
count_winter_ECA_vst <- vst_dds_grouping[(rownames(vst_dds_grouping) %in% ids_winter_ECA),]
...
count_winter_lower_vst <- vst_dds_grouping[(rownames(vst_dds_grouping) %in% ids_winter_lower),]
...
count_region_2seasons_vst <- vst_dds_grouping[(rownames(vst_dds_grouping) %in% ids_region_2seasons),]
...
```{r}
res_lower_affected_WvsF<- results(dds_grouping,contrast =
...
```{r}
res_lower_unaffected_WvsF<- results(dds_grouping,contrast =
...
```{r}
res_ECA_affected_WvsF<- results(dds_grouping,contrast =
...
```{r}
res_ECA_unaffected_WvsF<- results(dds_grouping,contrast =
...
count_regions.aff.WvsF_vst <- vst_dds_grouping[(rownames(vst_dds_grouping) %in% ids_regions.aff),]
...
count_com.region.aff.WvsF_vst <- vst_dds_grouping[(rownames(vst_dds_grouping) %in% ids_com.region.aff),]
...
count_com.region.unaff.WvsF_vst <- vst_dds_grouping[(rownames(vst_dds_grouping) %in% ids_com.region.unaff),]
...
count_uniq.region.unaff.WvsF_vst <- vst_dds_grouping[(rownames(vst_dds_grouping) %in% ids_uniq.region.unaff),]
...
count_uniq.region.aff.WvsF_vst <- vst_dds_grouping[(rownames(vst_dds_grouping) %in% ids_uniq.region.aff),]
...
count_all.com_WvsF_vst <- vst_dds_grouping[(rownames(vst_dds_grouping) %in% ids_all.com_WvsF),]
...
count_com.com_WvsF_vst <- vst_dds_grouping[(rownames(vst_dds_grouping) %in% ids_com.com_WvsF),]
...
count_all.sig_WvsF_vst <- vst_dds_grouping[(rownames(vst_dds_grouping) %in% ids_all.sig_WvsF),]
...
count_all_contrast_DEG_vst <- vst_dds_grouping[(rownames(vst_dds_grouping) %in% ids_all_contrast_DEG),]
...
```{r}
res_both_affvsunaff_fall<- results(dds_grouping, contrast = list(
...
```{r}
res_both_affvsunaff_winter<- results(dds_grouping, contrast = list(
...
```{r}
res_both_season<- results(dds_grouping, contrast = list(
...
```{r}
res_both_season_aff<- results(dds_grouping, contrast = list(
    c("SCTLD_RNA_Region_seasonSCTLD_affected_ECA_winter", "SCTLD_RNA_Region_seasonSCTLD_affected_Lower_winter"),
    c("SCTLD_RNA_Region_seasonSCTLD_affected_ECA_fall", "SCTLD_RNA_Region_seasonSCTLD_affected_Lower_fall")
  ),
  listValues = c(0.5, -0.5) )
...
```{r}
res_both_season_unaff<- results(dds_grouping, contrast = 
    c("SCTLD_RNA_Region_seasonSCTLD_unaffected_ECA_winter", "SCTLD_RNA_Region_seasonSCTLD_unaffected_Lower_winter"),
    c("SCTLD_RNA_Region_seasonSCTLD_unaffected_ECA_fall", "SCTLD_RNA_Region_seasonSCTLD_unaffected_Lower_fall"), 
    listValues =c(0.5,-0.5) )
...
res_affvsunaff <- results(dds_grouping, contrast = list(c(unaffected_conditions, affected_conditions)), 
               listValues = c(0.5,-0.5))
```

```{r}
res_affvsunaff<- results(dds_grouping, contrast = 
...
```{r}
res_affvsunaff_ECA<- results(dds_grouping, contrast = list(
...
count_affvsunaff_vst <- vst_dds_grouping[(rownames(vst_dds_grouping) %in% ids_affvsunaff),]
...
count_data_uniq_unaff<- vst_dds_grouping[(rownames(vst_dds_grouping) %in% genes_uniq_unaff),]
